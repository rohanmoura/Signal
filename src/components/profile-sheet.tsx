...
const ActionBtn: FC<ActionButtonProps> = ({
  Icon, label
}) => (
...
const ProfileSheet = ({
  ...
  return (
    <ScrollArea className='h-full'>
      ...
      >
        <DialogTrigger className='w-full' onClick={() => setBlockConfirmationDialog(true)}>
      ...
      <div className='flex flex-col gap-y-2'>
        ...
        <div>
          {groupsInCommon?.length && groupsInCommon.map(({ conversation }) => (
            <Link href={`/chats/${conversation._id}`} key={conversation._id} className='flex items-center space-x-3 hover:bg-gray-400 px-3 py-2 rounded-md'>
              <Avatar>
                <AvatarFallback>
